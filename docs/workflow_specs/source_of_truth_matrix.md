...
| 标签归一化与同义词映射 | `config/focus_tags.json` + `src/sciencemonitor/tags.py` + `src/sciencemonitor/llm.py` + `src/sciencemonitor/article_summaries.py` | `docs/workflow_specs/literature_note_style_guide.md`、`docs/workflow_specs/常用分级标签参考.md` | `focus_tags.json` 现在定义首选词表、canonical 命名、开放词表约束和候选记录路径；说明文档继续负责人工阅读和风格参考 |
