...
- 当前目标版本：`v1.3.0（大检查与优化中）`
- 当前备份版本：`v1.2.1`
- 当前结构治理状态：Phase 1 到 Phase 11 已完成；`chatgpt_web_manual` 工作流已完成并记录为 `v1.1.0`；agent skill、私有路径覆盖和格式审核治理已记录为 `v1.2.0`；当前正在做 `v1.3.0` 前的大检查与收束
- 当前 `docs/exec_plans/active/`：有进行中的计划
- 当前发布阻塞项：无
