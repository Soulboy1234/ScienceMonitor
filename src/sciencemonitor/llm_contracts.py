...
ARTICLE_PROMPT_REQUIREMENTS = [
    ...
    "7. 标签优先复用项目常用词表里的命名；如果确实没有合适标签，可以新造，但必须短、稳、具体、可检索，不能用交叉对比、趋势拟合、观测基准、模型调优这类宽泛动作词充当临时 tag。",
    "8. 常见 canonical 写法：地磁暴写 事件/磁暴；SC/sudden commencement 写 事件/磁暴/急始；HILDCAA 写 事件/HILDCAAs；Joule heating 写 对象/极区/焦耳加热；磁力计写 仪器/磁强计；TIE-GCM 写 模型/TIEGCM；E-CHAIM 这类模型写 模型/E-CHAIM；NO/一氧化氮等成分写 对象/热层/成分；空间天气预报一般写 应用/预测，只有明确 operational/业务化证据才写 应用/业务化预报。",
    "9. 仪器/卫星 tag 只有在论文实际使用具体仪器、任务或数据时才输出；不要输出 仪器/卫星、仪器/TIE-GCM 这类泛化或错类标签；参考文献、会议名、背景介绍或相关工作中偶然出现的缩写不能触发仪器 tag。",
    "10. F层/F2层这类宽泛电离层层位通常写 对象/电离层；只有明确是 foF2 或 hmF2 时才写 对象/电离层/foF2 或 对象/电离层/hmF2。",
    "11. 正文用一段中文说明：作者研究什么、用了什么数据/仪器、怎么做、来源文本明确给出的主要结果。第一句直接点明研究对象，不要先写泛泛背景。",
    "12. 如果来源文本信息不足，明确说证据有限，不要编造。",
    "13. 如果当前只拿到摘要和元数据，没有全文，必须按摘要级别概括；不要外推未明确给出的机制、应用效果或定量改进。",
    "14. 如果来源文本没有直接提到应用场景、业务价值或定量效果，不要主动补写“服务于……”或“可用于……”这类扩展。",
]
...
DEEP_READ_PROMPT_REQUIREMENTS = [
    ...
    "9. tag 只能来自题目、研究问题、数据方法和结果主线；不要根据局限性、后续问题、相关工作、参考文献、背景例子或用户研究偏好生成 tag。",
    "10. 地磁指数标签必须使用 canonical 名称：D指数、D index 或 Dst 写成 指数/Dst；K指数、Kp index、geomagnetic Kp 或 Kp≥/<= 写成 指数/Kp；但工程/数学语境中的 KP equation、generalized potential KP、Kadomtsev-Petviashvili 不是 Kp 指数，不能输出 指数/Kp。",
    "11. 仪器/卫星 tag 只有在论文实际使用该仪器、任务或数据时才输出；参考文献、会议名、背景介绍或相关工作中偶然出现的缩写不能触发仪器 tag。",
    "12. 常见 canonical 写法：地磁暴写 事件/磁暴；Joule heating 写 对象/极区/焦耳加热；磁力计写 仪器/磁强计；TIE-GCM 写 模型/TIEGCM；THEMIS 写 仪器/THEMIS_A-E；IMF By/Bz 写 指数/IMF/By 或 指数/IMF/Bz；NO/CO2/O 等成分不要写成仪器，写 对象/热层/成分；空间天气预报一般写 应用/预测，只有明确 operational/业务化证据才写 应用/业务化预报；卫星再入/轨道衰减写 应用/卫星轨道衰减；PINN/PINNs/物理信息神经网络写 方法/建模/机器学习/PINN；XAI/可解释性人工智能写 方法/可解释模型/XAI。",
    "13. 如果证据边界明显，要在局限或需要人工复核中明确写出，不要编造。",
    "14. 一句话总述必须直接说明研究目标、关键数据/方法和核心结果，优先用“这篇文章针对……”“这篇文章关注……”或“作者研究的是……”，不要用“不是……而是……”这类对比式开头。",
    "15. 除专有名词、仪器名、模型名、指数名、变量名和期刊名外，正文尽量使用中文，不要大量中英混写。",
    "16. 不要输出文件名、目录归档建议、证据范围、Research question、Core claims、Method summary 等旧版区块名。",
    "17. “为什么做”和“新意与贡献”也不要写“不是……而是……”这类对比式句型，直接陈述研究目标或贡献。",
    "18. 中文语境下如需强调术语，优先使用双引号，不要用单引号。",
    "19. “为什么做”必须紧扣文章直接研究问题，不要主动外推到热层密度、卫星阻力、业务化应用等下游问题，除非论文正文明确把它们作为研究目标。",
    "20. “和我已有工作的关系”只说明与用户研究主线最直接的关系；如果联系是间接的，就明确写“间接相关”，不要主动扩展到业务化风险评估、任务规划或工程影响。",
    "21. 如果“关键结果”“新意与贡献”“局限性”包含多个要点，优先输出为 1. 2. 3. 的分条形式。",
    "22. “为什么做”优先依据 Introduction / 引言 中的研究背景、已有认识不足、争议点和作者要填补的空白来总结，不要只把研究目标压成一句短句。",
    "23. 如果某一段内部需要分点，编号必须单独换行写成 1. 2. 3.，不要把“原因。1.”、“结论。1.”这种连写放在同一行。",
]
...
def build_ollama_deep_read_evidence_prompt_v2(
    ...
    return "\n".join(
        [
            ...
            "8. 如果论文是 PINN/XAI/工程 AI 方法批判或综述，证据包必须单独覆盖：PINN 嵌入简化物理的失败模式、XAI 相关性归因与物理因果的冲突、两者叠加的误差传播、所有明确案例研究、作者给出的验证/部署建议。",
            "",
            *preference_lines,
            "",
            f"题目：{metadata.get('title', '')}",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "要求：",
            *DEEP_READ_PROMPT_REQUIREMENTS,
            "24. 关键结果四类中，除非证据确实不足，每类至少写 2 条；如果不足，要在该类和需要人工复核中说明证据不足。",
            "25. 局限性至少区分论文自身局限和本次全文抽取/证据边界；可复现性要说明能复核什么、不能复现实算什么。",
            "26. “和我已有工作的关系”必须保守。若论文没有直接研究热层密度、热层风、卫星阻力、业务化预报或空间环境风险，就明确写“间接相关”。",
            "27. relation 字段必须解释本文相对已有工作的承接、差异或补充证据，不要只输出“直接相关/间接相关/弱相关/不相关”这类等级词。",
            "28. 输出字段内容要有信息密度，避免每个字段只有一句泛泛判断。",
            "29. 如果论文是 PINN/XAI/工程 AI 方法批判或综述，最终报告必须覆盖：PINN 的简化方程/多尺度/边界/优化问题，XAI 的相关性而非因果机制问题，乘法式误差传播或虚假可信度，至少两个明确案例研究，以及作者建议的验证、敏感性分析、独立审查或 fail-safe 边界。",
            "",
            *preference_lines,
            "",
            f"题目：{metadata.get('title', '')}",
    ...
    return "\n".join(
        [
            ...
            *[f"- {issue}" for issue in issues],
            ...
            "4. 修正 tags：标签是检索系统，不是观点摘抄；只能来自题目、研究问题、数据方法和结果主线，不要根据局限性、后续问题、相关工作或参考文献生成；对象/ 只放实体、区域、系统、观测量或明确物理对象，不要写 对象/物理机制、对象/能量转换、对象/过程、对象/现象、对象/空间天气、对象/评估、对象/卫星；D指数/D index 写成 指数/Dst；K指数/Kp index/geomagnetic Kp 写成 指数/Kp，但 KP equation/generalized potential KP 不是 Kp 指数；TIE-GCM 写 模型/TIEGCM；THEMIS 写 仪器/THEMIS_A-E；IMF By/Bz 写 指数/IMF/By 或 指数/IMF/Bz；NO/CO2/O 等成分不要写成仪器，写 对象/热层/成分；PINN 写 方法/建模/机器学习/PINN；XAI 写 方法/可解释模型/XAI；仪器 tag 必须有论文实际使用该仪器或数据的证据。",
